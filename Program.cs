    ...
    internal static class Program
    {
        ...
        private static void DeleteCngCertificate(bool fUser, string Thumbprint)
        {
            ...
            foreach (X509Certificate2 cert in store.Certificates)
            {
                ...
                using var priv_key_rsa = (RSACng)cert.GetRSAPrivateKey();
                if (priv_key_rsa != null)
                    DeleteIfOnTPM(store, cert, priv_key_rsa.Key);

                using var priv_key_ecdsa = (ECDsaCng)cert.GetECDsaPrivateKey();
                if (priv_key_ecdsa != null)
                    DeleteIfOnTPM(store, cert, priv_key_ecdsa.Key);

#if False
                using var priv_key_ecdh = (ECDiffieHellmanCng)cert.GetECDiffieHellmanPrivateKey();
                if (priv_key_ecdh != null)
                    DeleteIfOnTPM(store, cert, priv_key_ecdh.Key);
#endif
                return;
            }
        }
...
#if False
                using var ecdhPrivKey = (ECDiffieHellmanCng)cert.GetECDiffieHellmanPrivateKey();
                if (ecdhPrivKey != null)
                {
                    if (!ecdhPrivKey.Key.ExportPolicy.HasFlag(CngExportPolicies.AllowPlaintextExport))
                        AllowPlaintextExportWorkaround(ecdhPrivKey);

                    algorithm = CngAlgorithm.ECDiffieHellman;
                    keyData = ecdhPrivKey.Key.Export(CngKeyBlobFormat.GenericPrivateBlob);
                }
#endif
            }
...
#if False
                    {
                        using var keyOfCopiedCertificate = (ECDiffieHellmanCng)cngCert.GetECDiffieHellmanPrivateKey();
                        if (keyOfCopiedCertificate != null)
                            PrintExportPolicy(keyOfCopiedCertificate.Key);
                    }
#endif
